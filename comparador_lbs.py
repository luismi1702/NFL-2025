"""
comparador_lbs.py
Radar chart comparando dos Linebackers (ILB/MLB) en 6 dimensiones.

Métricas:
  1. Tackles totales (solo + asistencias)
  2. Tackles for loss
  3. Sacks
  4. QB hits
  5. Pases defendidos
  6. EPA on/off (pbp_participation) — impacto defensivo global
"""
...
def find_player(query, lb_stats, nm_col):
    matches = lb_stats[lb_stats[nm_col].str.lower().str.contains(query.lower(), na=False)]
    if matches.empty:
        raise SystemExit(f"LB '{query}' no encontrado. Prueba con otra parte del nombre.")
...
def compute_lb_metrics(player_name, stats_row, pbp_df, part_df):
    tackles = float(stats_row.get("def_tackles_total", float("nan")))
...
# ── INPUT ──────────────────────────────────────────────────────────────────────
p1_input = input("LB 1 (apellido o nombre parcial, p.ej. Wagner): ").strip()
p2_input = input("LB 2 (apellido o nombre parcial, p.ej. White): ").strip()
...
to_num(df_stats, ["def_tackles", "def_tackle_assists", "def_tackles_for_loss",
                  "def_sacks", "def_qb_hits", "def_pass_defended", "games"])
...
lb_stats = df_stats[
    df_stats["position"].isin(LB_POSITIONS) &
    (df_stats["def_tackles_total"] >= MIN_TACKLES)
].copy()
print(f"LBs con ≥{MIN_TACKLES} tackles: {len(lb_stats)}")
...
# ── FIND PLAYERS ───────────────────────────────────────────────────────────────
nm_col = pick_col(lb_stats, "player_name", "player_display_name")
p1_name, p1_stats_row = find_player(p1_input, lb_stats, nm_col)
p2_name, p2_stats_row = find_player(p2_input, lb_stats, nm_col)
...
qualified_names = lb_stats[nm_col].tolist()
...
for name in qualified_names:
    row = lb_stats[lb_stats[nm_col] == name]
    if row.empty:
        continue
...
fig.text(0.5, 0.92,
         f"Comparación radar — 6 dimensiones | Normalizadas entre ILB/MLB con ≥{MIN_TACKLES} tackles",
