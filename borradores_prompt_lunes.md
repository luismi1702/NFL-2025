...
de PARTIDO de esta jornada en `{DIR}/borradores_posts.md`.
...
partido ya estan generados. Tu trabajo es elegir las TRES mejores historias y
escribirlas.
...
## Como elegir las tres historias
...
3. **Extremos y jugadores**, si son de verdad extremos.

No escribas tres posts del mismo partido ni tres del mismo tipo.

## Reglas innegociables

1. **Cada numero sale de `destacados.txt`.** Si un numero no esta ahi, no
   existe. Prohibido completar de memoria y prohibido leerlo de un PNG a ojo.
...
8. **Cierre: hashtags y menciones**, en este orden y formato:
   ...
   - Maximo 2-3 menciones. Solo en posts sobre UN equipo concreto.
9. **Elige bien la imagen** de cada post, que no todas cuentan lo mismo:
   - la **ficha** si el post va de nivel ("fue excepcional", "fue de los peores")
     o de reparto de facetas;
   - las **claves** si va de que un equipo cambio respecto a lo suyo;
   - el **boxscore** si va de la historia del partido (remontada, final ajustado);
   - `under_center` si va del cambio de formacion en la liga.

## Que entregar en `{DIR}/borradores_posts.md`
...
- Justo debajo, una linea `IMAGEN: nombre_del_png` (solo el nombre, sin ruta).
...
IMAGEN: 13_ficha_GB_vs_MIN_2026_w01.png
...
Tres secciones, con dos alternativas cada una y angulos distintos, no la misma
frase retocada. Si `destacados.txt` falta o esta vacio, dilo y no inventes:
...
`{DIR}/borradores_posts.md`. No termines sin haberlo escrito.
