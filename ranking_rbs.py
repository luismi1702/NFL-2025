...
ZOOM_MIN = 0.021   # legible incluso para el RB con menos acarreos
...
OUT = salida(f"scatter_RB_RZ_vs_3rd_{SEASON}.png", SEASON)
...
for (rb_key, row), ly, off, zoom in zip(stats.iterrows(), lys, offs, zooms):
    x    = row["rz_epa"]
    y    = row["d3_epa"]
    team = row["team"]
    name = row["label"]
...
fig.text(0.5, 0.01,
         f"Fuente: nflverse-data  ·  mín. {MIN_RZ} acarreos en RZ y {MIN_3RD} en 3er down  ·  Líneas = media de la muestra  ·  Tamaño del logo = nº de acarreos",
